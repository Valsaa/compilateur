...
@init{TableSymboles ts = new TableSymboles(); code = new Code();}
: ^(PROGRAMME (c=unite[ts] {
    code.append(c);
  })+)
;
...
: ^(PARAMETRE (c=parametre[ts] {codes.add(c);})*)
;
...
conditionnelle [TableSymboles ts] returns [Code code]
@init{code = new Code();}
: ^(IF_KW exp=expression[ts] {
